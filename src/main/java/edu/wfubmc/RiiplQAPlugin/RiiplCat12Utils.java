...
/** A class of utilities dealing with CAT12 processing
* @author Ben Wagner
* @author ANSIR Lab
* @version 0.1 January 2012
*/
public class RiiplCat12Utils
{
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(RiiplCat12Utils.class);

	/** Sets reads the tissue segmentation report (wfu_cat12_tissue_report.m) and places in cat12TissueReport
	...
	*/
	...
	/** Populates the normalization QC for NormCat128[DTI|PASL|PCASL|MTR|...]Data 
	...
	*/
	public static void normalizationQC(org.nrg.xdat.om.XnatMrsessiondata mr, RunData data, ItemI item, String elementName, String expLabel, String csvFileName, ArrayList<WildcardFileFilter> imageFilters, boolean rpProcess)
	{
		...
		File cat12Dir = new File(subjectDir+"/nifti/cat12");
		File imageDir = cat12Dir;
...
//				logger.info("Has username in index: " +qcDataIndex);
		...
		try
		{
			if (!cat12Dir.exists())
			{
				logger.error(subjectDir+"/nifti/cat12 does not exist");
				data.setMessage(subjectDir+"/nifti/cat12 does not exist");
				data.setScreenTemplate("Error.vm");
				return;
			}
			FileFilter fileFilter = new WildcardFileFilter("p*_seg8.txt");
			File[] seg8TxtFiles = cat12Dir.listFiles(fileFilter);
			
			if (seg8TxtFiles.length == 0)
			{
				logger.error("No p*_seg8.txt files found in "+subjectDir+"/nifti/cat12");
				data.setMessage("No p*_seg8.txt files found in "+subjectDir+"/nifti/cat12");
				data.setScreenTemplate("Error.vm");
				return;
			}
			else if (seg8TxtFiles.length > 1)
			{
				logger.error("Too many p*_seg8.txt files found in "+subjectDir+"/nifti/cat12: Count:"+seg8TxtFiles.length);
				data.setMessage("Too many p*_seg8.txt files found in "+subjectDir+"/nifti/cat12: Count:"+seg8TxtFiles.length);
				data.setScreenTemplate("Error.vm");
				return;
			}
		...
		try
		{
			...
			if (elementName.equals("riiplqa:NormCat12DTIData"))
			{
				//look for DTI_SNNNN directories first.
				//Also for DTI_SNNN directories which come in normalized from the older DTI pipeline
				File[] DTIdirs = cat12Dir.listFiles((FileFilter) new WildcardFileFilter((new String[] {"DTI_S????","DTI_S???"})));
				logger.debug("number of nifti/cat12/DTI_Sxxx directories: "+DTIdirs.length);
				if (DTIdirs.length == 1)
				{
				...
				{
					logger.error("More than one CAT12/DTI_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setMessage("More than one CAT12/DTI_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setScreenTemplate("Error.vm");
					return;
				}
			}
			if (elementName.equals("riiplqa:NormCat12DKIData"))
			{
				//look for DKI_SNNNN directories first.
				//Also for DKI_SNNN directories which come in normalized from the older DKI pipeline
				File[] DKIdirs = cat12Dir.listFiles((FileFilter) new WildcardFileFilter((new String[] {"DKI_S????","DKI_S???"})));
				logger.debug("number of nifti/cat12/DKI_Sxxx directories: "+DKIdirs.length);
				if (DKIdirs.length == 1)
				{
				...
				{
					logger.error("More than one CAT12/DKI_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setMessage("More than one CAT12/DKI_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setScreenTemplate("Error.vm");
					return;
				}
			}
			if (elementName.equals("riiplqa:NormCat12DKEData"))
			{
				//look for DKE_SNNNN directories first.
				//Also for DKE_SNNN directories which come in normalized from the older DKE pipeline
				File[] DKEdirs = cat12Dir.listFiles((FileFilter) new WildcardFileFilter((new String[] {"DKE_S????","DKE_S???"})));
				logger.debug("number of nifti/cat12/DKE_Sxxx directories: "+DKEdirs.length);
				if (DKEdirs.length == 1)
				{
				...
				{
					logger.error("More than one CAT12/DKE_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setMessage("More than one CAT12/DKE_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setScreenTemplate("Error.vm");
					return;
				}
			}
			if (elementName.equals("riiplqa:NormCat12MTRData"))
			{
				//look for MTR_SNNNN directories first.
				File[] MTRdirs = cat12Dir.listFiles((FileFilter) new WildcardFileFilter((new String[] {"MTR_S????","MTR_S???"})));
				logger.debug("number of nifti/cat12/MTR_Sxxxx directories: "+MTRdirs.length);
				if (MTRdirs.length == 1)
				{
				...
				{
					logger.error("More than one CAT12/MTR_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setMessage("More than one CAT12/MTR_Snnnn directory in "+subjectDir+"/nifti/cat12");
					data.setScreenTemplate("Error.vm");
					return;
				}
			}
			if (elementName.equals("riiplqa:NormCat12LST2Data"))
			{
				processCsvFileTissue = false; //No processing of TissueData
				//look for MTR_SNNNN directories first.
				File[] LSTdirs = cat12Dir.listFiles((FileFilter) new WildcardFileFilter((new String[] {"lst8"})));
				logger.debug("number of nifti/cat12/lst8 directories: "+LSTdirs.length);
				if (LSTdirs.length == 1)
				{
...
//*********************************************************************************
				...
				{
					logger.error("More than one CAT12/LST2 directory in "+subjectDir+"/nifti/cat12");
					data.setMessage("More than one CAT12/LST2 directory in "+subjectDir+"/nifti/cat12");
					data.setScreenTemplate("Error.vm");
					return;
				}
			}
			...
			if (processCsvFileTissue)
			{
				if (csvFile==null)
				{
					csvFile = new File(subjectDir+"/nifti/cat12/"+csvFileName);
				}
				RiiplCat12Utils.processTissueReport(data,item,eName,csvFile);
			}
		...
		try
		{
			...
			else
			{
				...
				for (int sX=0; sX < imageFilters.size(); sX++)
				{
					...
					if (sX==0)
					{
						niftis = cat12Dir.listFiles((FileFilter)(imageFilters.get(sX)));
					}
					else
					{
					...
					{
						if (elementName.equals("riiplqa:NormCat12vPCASLData"))
						{
					...
					{
						logger.error("Unable to find image: " + cat12Dir + "/" + imageFilters.get(sX));
						data.setMessage("Unable to find image: " + cat12Dir + "/" + imageFilters.get(sX));
						data.setScreenTemplate("Error.vm");
						return;
					}
		...
		try
		{
			if (rpProcess)
			{
				...
				*/
				...
				Collection possibleRpFiles = FileUtils.listFiles(cat12Dir, new WildcardFileFilter("rp_*"+fileBase+"*.txt"), new WildcardFileFilter(searchDirs));
...
//				data.setScreenTemplate("Error.vm");
	...
	/** Populates the normalization QC for NormCat12[DTI|PASL|PCASL|MTR|...]Data 
	* @param		data					The "RunData" or Template session information
	* @param		item					The Datatype's DATA
	* @param		elementName		element name in the XSD
	* @param		expLabel			The experiment labeL suffix (will be appended to mr.getProperty("label"))
...
//				logger.info("Has username in index: " +qcDataIndex);
	...
	/** Checks the CAT12 data type for file changes.
	* @param		data					The "RunData" or Template session information
	* @param		item					The Datatype's DATA
	* @param		eName					The element name in the XSD
	...
	*/
	...
	/** Checks the Cat12.properies data file for nifti/cat12 location
	* @param		project				The project ot lookup
	* @return									String for nifti/cat12 location
	*/
	public static String getCat12Dir(String project) throws Exception, IOException
	{
		Properties prop = new Properties();
		try
		{
			//load a properties file
			prop.load(new FileInputStream(XFT.GetConfDir()+"Cat12.properties"));
		}
		...
		logger.debug("Read " + prop.size() + " cat12 project mappingfrom properties file");
